...
export const orderByYearThenByMonthThenByDayThenHour = (a: Rota, b: Rota) => {
  const departureDate_a = a.departureDate.split(
    a.departureDate.includes(",") ? "," : "/"
  );
  const departureDate_b = b.departureDate.split(
    b.departureDate.includes(",") ? "," : "/"
  );
...
export const getDurationTimeInGoogleMaps = async (
  origin: any,
  destination: any
) => {
  const response = await axios.get(
    `https://maps.googleapis.com/maps/api/distancematrix/json?units=metric&origins=${origin}&destinations=${destination}&key=AIzaSyCn_wshzahv0Ta84NiC3qrxeAnJL8U3p70`,
    {
      headers: {
